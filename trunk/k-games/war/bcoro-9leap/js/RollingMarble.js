...
(function(Class) {
	...
	Class.prototype.start = function() {
		...
		var data = StageData.MAP[stageName];
		if (data == null) {
	...
	Class.prototype.goal = function() {
		...
		self.score += self.stageNo*2000 + self.time/10;
		//alert(self.score+":"+self.time);
...
*/
	...
	Class.prototype.exit = function() {
		var self = this;
		var result = "Stage "+(self.stageNo)+"\nScore: "+self.score;
		if (self.stageNo >= self.stageNames.length) {
			result += "\nAll stage clear!!";
		}
		//alert(this.score+":"+result);
		end9leap(this.score, result);
	}
